...
#mk=data.frame(gcContent,uncorrectedRD)
#	write.table(mk,"uncorrected.txt")
#       mk1=data.frame(gcContent,correctedRD)
#	write.table(mk1,"corrected.txt")
...
# start plotting now 
	#par(mfrow = c(2,1));
	plot(gcContent,uncorrectedRD,main = "Read Depth vs GC-content",col="red",cex=4,xlim=c(0,1),ylim=c(0,3*medURD),pch='.',xlab="GC fraction",ylab="Read Depth");
	#lines(lowess(gcContent,uncorrectedRD),col="green",lwd=2);
	points(gcContent,correctedRD,col="blue",cex=4,ylim=c(0,3*medRD),pch='.',xlab="GC fraction",ylab="Read Depth");
